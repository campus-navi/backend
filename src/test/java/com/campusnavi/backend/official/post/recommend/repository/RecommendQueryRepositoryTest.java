...
@DataJpaTest
@ActiveProfiles("test")
@Import({JpaConfig.class, QueryDslConfig.class, RecommendQueryRepository.class})
@EnableConfigurationProperties(RecommendProperties.class)
class RecommendQueryRepositoryTest extends PostgresSliceTestSupport {
    ...
    class FindStatsByPostIds {
        ...
        @Test
        @DisplayName("같은 학과 단일 조회자가 있으면 정상 집계된다")
        void success() {
            // given
            ...
            // when
            Map<Long, OfficialPostStatsRaw> stats = byPostId(recommendQueryRepository.findStatsByPostIds(
        ...
        @Test
        @DisplayName("view_count가 viewCap을 초과하면 cap된 값이 합산된다")
        void viewCap() {
            // given
            OfficialPost post = persistRecommendablePost(LocalDate.now());
        ...
        @Test
        @DisplayName("요청자 학과에 속하지 않은 회원의 조회는 제외된다")
        void otherDept() {
            // given
            OfficialPost post = persistRecommendablePost(LocalDate.now());
        ...
        @Test
        @DisplayName("이중 전공 회원이 여러 학과에 속해도 중복 카운트되지 않는다")
        void doubleMajor() {
            // given
            OfficialPost post = persistRecommendablePost(LocalDate.now());
        ...
        @Test
        @DisplayName("WITHDRAWN 회원의 조회는 제외된다")
        void withdrawn() {
            // given
            OfficialPost post = persistRecommendablePost(LocalDate.now());
        ...
        @Test
        @DisplayName("빈 postIds나 빈 requesterDeptIds면 빈 리스트를 반환한다")
        void emptyInput() {
            // when / then
            assertThat(recommendQueryRepository.findStatsByPostIds(
                    List.of(), 2022, 3, List.of(requesterDept.getId()))).isEmpty();
            assertThat(recommendQueryRepository.findStatsByPostIds(
                    List.of(1L), 2022, 3, List.of())).isEmpty();
        }
    }
    ...
    class FindRecommendCandidates {
        ...
        @Test
        @DisplayName("tag.isRecommendable=false인 글은 제외된다")
        void notRecommendable() {
            ...
            List<OfficialPostRecommendCandidateRaw> result =
                    recommendQueryRepository.findRecommendCandidates(condition(), LocalDateTime.now().plusDays(1), 0L);
        ...
        @Test
        @DisplayName("publishedAt이 신선도 이전인 글은 제외된다")
        void stale() {
            // given
        ...
        @Test
        @DisplayName("aiMeta.endDate가 지난 글은 제외된다 (null이면 포함)")
        void expired() {
            // given
            LocalDate today = LocalDate.now();
        ...
        void crawledAtCutoff() {
            // given
            LocalDateTime slotAt = LocalDateTime.now().minusHours(1);
            OfficialPost beforeSlot = persistPostWithCrawledAt(
                    recommendableTag, LocalDate.now(), null, slotAt.minusMinutes(10));
            OfficialPost afterSlot = persistPostWithCrawledAt(
                    recommendableTag, LocalDate.now(), null, slotAt.plusMinutes(10));

            // when
            List<OfficialPostRecommendCandidateRaw> result =
                    recommendQueryRepository.findRecommendCandidates(condition(), slotAt, 0L);
        ...
        @Test
        @DisplayName("freshness 윈도우는 slotAt 기준으로 산정된다")
        void freshnessRelativeToSlot() {
            // given
            LocalDateTime slotAt = LocalDate.now().minusDays(1).atTime(18, 0);
            LocalDate slotDate = slotAt.toLocalDate();
            OfficialPost insideWindow = persistPostWithCrawledAt(
                    recommendableTag, slotDate.minusDays(10), null, slotAt.minusHours(1));
            OfficialPost outsideWindow = persistPostWithCrawledAt(
                    recommendableTag, slotDate.minusDays(20), null, slotAt.minusHours(1));
            ...
            // then
            assertThat(result).extracting(OfficialPostRecommendCandidateRaw::postId)
                    .contains(insideWindow.getId())
                    .doesNotContain(outsideWindow.getId());
        }

        @Test
        @DisplayName("요청자가 이미 본 글은 후보에서 제외된다")
        void excludeViewed() {
            // given
            OfficialPost viewedPost = persistRecommendablePost(LocalDate.now());
            OfficialPost unviewedPost = persistRecommendablePost(LocalDate.now());
            Member requester = persistActiveMember(2022, 3, requesterDept);
            persistView(requester.getId(), viewedPost.getId(), 1);

            // when
            List<OfficialPostRecommendCandidateRaw> result = recommendQueryRepository
                    .findRecommendCandidates(condition(), LocalDateTime.now().plusDays(1), requester.getId());

            // then
            assertThat(result).extracting(OfficialPostRecommendCandidateRaw::postId)
                    .contains(unviewedPost.getId())
                    .doesNotContain(viewedPost.getId());
        }
    }

    @Nested
    @DisplayName("findCardsByIds")
    class FindCardsByIds {

        @Test
        @DisplayName("빈 입력이면 빈 리스트를 반환한다")
        void emptyInput() {
            // when / then
            assertThat(recommendQueryRepository.findCardsByIds(List.of())).isEmpty();
            assertThat(recommendQueryRepository.findCardsByIds(null)).isEmpty();
        }

        @Test
        @DisplayName("주어진 ID에 해당하는 글만 반환한다")
        void inIds() {
            // given
            OfficialPost p1 = persistRecommendablePost(LocalDate.of(2026, 5, 12));
            OfficialPost p2 = persistRecommendablePost(LocalDate.of(2026, 5, 12));

            // when
            List<OfficialPostCardRaw> result = recommendQueryRepository
                    .findCardsByIds(List.of(p1.getId(), p2.getId(), 9999L));
        ...
        @Test
        @DisplayName("프로젝션 필드가 정확히 매핑된다")
        void projectionFields() {
            // given
            LocalDate publishedAt = LocalDate.of(2026, 5, 1);
            OfficialPost post = persistPostWithTag(recommendableTag, publishedAt, null);

            // when
            List<OfficialPostCardRaw> result =
                    recommendQueryRepository.findCardsByIds(List.of(post.getId()));

            // then
            assertThat(result).hasSize(1);
            OfficialPostCardRaw raw = result.get(0);
            assertThat(raw.postId()).isEqualTo(post.getId());
            assertThat(raw.title()).isEqualTo("title");
            assertThat(raw.tagName()).isEqualTo(recommendableTag.getName());
            assertThat(raw.summary()).isEqualTo("summary");
            assertThat(raw.publishedAt()).isEqualTo(publishedAt);
            assertThat(raw.s3Key()).isNull();
        }
    }
    ...
    private OfficialPost persistPostWithTag(Tag tag, LocalDate publishedAt, LocalDate endDate) {
        return persistPostWithCrawledAt(tag, publishedAt, endDate, LocalDateTime.now());
    }

    private OfficialPost persistPostWithCrawledAt(
            Tag tag, LocalDate publishedAt, LocalDate endDate, LocalDateTime crawledAt) {
