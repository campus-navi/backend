...
sequenceDiagram
    ...
    Scheduler->>Crawler: runAllScheduled()
...
AI 메타 처리는 이 스케줄러 풀과 별개로 `aiMetaExecutor`에서, 어드민 장기작업(시드 크롤링·AI 배치)은
`adminTaskExecutor`에서 처리한다(아래 "어드민 장기작업 비동기화" 참조).
...
### 어드민 장기작업 비동기화
시드 크롤링(전체 게시판 초기 수집)과 AI 메타 수동 배치는 수십 분까지 걸릴 수 있어, 동기 응답으로는
CloudFront origin timeout(약 30초)에 걸려 504가 나고 톰캣 요청 스레드를 장기 점유한다. 두 어드민
API는 요청 스레드에서 인메모리 가드(AtomicBoolean CAS)를 획득한 뒤 전용 풀 `adminTaskExecutor`
(2스레드)에 작업을 제출하고 **202 Accepted를 즉시 반환**한다. 실행 중 재요청은 409로 거부하고,
진행 상태는 상태 조회 API(크롤링: 실행 여부·시각 / AI 배치: 실행 여부·상태별 카운트)로 확인한다.
크롤링 가드는 스케줄 크롤링·실패 재시도와 공유해, 같은 공지를 두 경로가 동시에 크롤하다
unique 위반으로 재시도 횟수를 헛되이 소진하는 경합을 막는다(겹치면 스케줄 쪽이 skip).

AI 배치는 PENDING 대상을 10건 청크로 잘라 FastAPI를 순차 호출한다. 수십건 1회 호출 대비
호출당 응답이 response-timeout(300초) 안에 안정적으로 들어오고, 호출 실패 시 FAILED 처리 범위가
청크로 한정되며, 첨부 presigned URL(TTL 1시간)도 청크 처리 직전에 생성돼 만료 위험이 없다.
가드·실행 시각은 인메모리라 단일 인스턴스 전제이며, 다중 인스턴스 확장 시 분산락으로 교체한다.
...
미열람 여부는 조회 시점에 `official_post_view`와 차집합으로 계산한다.

item 테이블로 정규화할 때의 실질 이점은 `post_id`에 FK를 걸어 얻는 참조 무결성인데, 해당 도메인에서는 이점을 얻지 못한다.
해당 도메인은 읽기 경로가 삭제 여부와 무관하게 **만료(마감 지난) 공지**를 걸러야 해서 postId 재검증(`findValidPostIdsByIds`)이 
정규화 여부와 관계없이 필요하고, 이 재검증이 삭제돼 존재하지 않는 공지 ID까지 함께 걸러낸다. 
즉 참조 무결성은 DB 제약이 아니라 읽기 시점 재검증이 보장한다. postId 역방향 조회
("이 공지를 놓친 회원 목록")나 항목 단위 집계 요구가 생기면 그때 정규화로 전환한다.
